...
from services.comprehensive_analysis_service import ComprehensiveAnalysisService, analyze_custom_prompt
...
def analyze_lead_behavior(lead_id):
    try:
        ...
        current_app.logger.info(f"Received request data: {data}")  # 受け取ったデータをログに出力して確認
        ...
        if custom_prompt:
            current_app.logger.info(f"# カスタムプロンプトを使用した分析: {custom_prompt}")

            # カスタムプロンプトを使用した分析
            analysis_results = analysis_service.analyze_lead_data_with_custom_prompt(lead_id, custom_prompt)
        else:
            current_app.logger.info(f"# 標準パラメータを使用した分析 custom_prompt= {custom_prompt}")
            # 標準パラメータを使用した分析
