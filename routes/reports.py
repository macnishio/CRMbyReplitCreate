...
def index():
    ...
    lead_scores = db.session.query(func.floor(Lead.score / 10) * 10, func.count(Lead.id)).filter_by(user_id=current_user.id).group_by(func.floor(Lead.score / 10) * 10).all()
